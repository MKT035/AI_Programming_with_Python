...
def process_data(data_dir):
    ...
    loaders = {'train':train_loader,'valid':valid_loader,'test':test_loader,'labels':cat_to_name}
    return loaders
    ...
    for e in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(train_loader):
            ...
            if steps % print_every == 0:
                valid_accuracy = test_accuracy(model,valid_loader,device)
                print("Epoch: {}/{}... ".format(e+1, epochs),
                      "Loss: {:.4f}".format(running_loss/print_every),
                      "Validation Accuracy: {}".format(round(valid_accuracy,4)))            
                running_loss = 0
    print("DONE TRAINING!")
    test_result = test_accuracy(model,test_loader,device)
